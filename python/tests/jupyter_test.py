...
# Python magics template filename. In theory, this would be used to comment out
# autogenerated calls to get_ipython (which cause tests to fail unless run with
# ipython, which introduces further issues). In practice, recent versions of
# nbconvert (currently 7) seem to be having a variety of issues with templates.
# Since the templates use case was so simple, I've temporarily swapped it for
# a sed call while nbconvert shakes out the issues.
...
def _check_notebook_style(path):
    ...
    try:
        # See note on MAGICS_TEMPLATE
        args = ["sed", "-i", 's/get_ipython/# get_ipython/g', testname]
        subprocess.check_call(args)
    except subprocess.CalledProcessError:
        return ["Failed to comment out ipython magic using sed for {}".format(testname)]
