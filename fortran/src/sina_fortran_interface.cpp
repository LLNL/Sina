...
extern "C" void create_document_and_record_(char * recID)
{
  ...
  sina::ID id{recID, sina::IDType::Global};
  std::unique_ptr<sina::Record> myRecord{new sina::Record{id, "my_type"}};
  sina_document->add(std::move(myRecord));
}

extern "C" sina::Record *Sina_Get_Record()
{
    if(sina_document)
    {
        sina::Document::RecordList const &allRecords = sina_document->getRecords();
        if(allRecords.size())
        {
            std::unique_ptr<sina::Record> const &myRecord = allRecords.front();
            return myRecord.get();
        }
    }
    return nullptr;
...
extern "C" void sina_add_logical_(char * key, bool *value, char *units, char *tags)
{
    if (sina_document)
    {
        sina::Record *sina_record = Sina_Get_Record();
        std::string key_name = std::string(key);
        if(sina_record)
        {
            sina::Datum datum{static_cast<double>(*value)};
            if (units)
            {
...
extern "C" void write_sina_document_(char * input_fn)
{
    ...
    if(sina_document)
    {
        sina::saveDocument(*sina_document, filename.c_str());
    }
}
...
extern "C" void sina_add_curveset_(char *name)
{
    if (sina_document)
    {
        ...
        if(sina_record)
        {
            sina::CurveSet cs{name};
            sina_record->add(cs);
        }
    }
}

extern "C" void sina_add_curve_long_(char *curveset_name, char *curve_name, long long int *values, int *n, bool *independent)
{
    if (sina_document)
    {
        sina::Record *sina_record = Sina_Get_Record();
        if(sina_record)
        {
            ...
            sina::Curve curve{curve_name, y, static_cast<size_t>(*n)};

            auto &curvesets = sina_record->getCurveSets();
            sina::CurveSet cs=curvesets.at(curveset_name);
            if (*independent)
            {
...
extern "C" void sina_add_curve_int_(char *curveset_name, char *curve_name, int *values, int *n, bool *independent)
{
    if (sina_document)
    {
        sina::Record *sina_record = Sina_Get_Record();
...
extern "C" void sina_add_curve_float_(char *curveset_name, char *curve_name, float *values, int *n, bool *independent)
{
    if (sina_document)
    {
        sina::Record *sina_record = Sina_Get_Record();
...
extern "C" void sina_add_curve_double_(char *curveset_name, char *curve_name, double *values, int *n, bool *independent)
{
    if (sina_document)
    {
        sina::Record *sina_record = Sina_Get_Record();
        if(sina_record)
        {
            sina::Curve curve{curve_name, values, static_cast<size_t>(*n)};
